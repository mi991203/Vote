<%@ Page Title="" Language="C#" MasterPageFile="~/MasterPageForAdmin.Master" AutoEventWireup="true"
    CodeBehind="VoteDetailInfoManage.aspx.cs" Inherits="MyWeb.VoteDetailInfoManage" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
    <%--<div>
        <a href="AddVoteDetailInfo.aspx">添加项目</a>
    </div>--%>
    <input type="hidden" id="voteId" runat="server" />
    <div>
        <table class="tbl">
            <thead>
                <tr>
    ...
    <script type="text/javascript">
        ...
        var voteId = $("#ContentPlaceHolder1_voteId").val();
        $.ajax({
            type: "post",
            dataType: "json",
            contentType: "application/json", //注意：WebMethod()必须加这项，否则客户端数据不会传到服务端
            data: "{ 'pi': '" + pi + "','ps': '" + ps + "','voteId': '" + voteId + "'}", //注意：data参数可以是string个int类型
            url: "VoteDetailInfoManage.aspx/loadVoteDetailInfoManage", //模拟web服务，提交到方法
        ...
        $(".tbl").on("click", ".btn", function () {
            ...
            $.ajax({
                ...
                url: "VoteDetailInfoManage.aspx/RemoveVoteDetailInfoManage", //模拟web服务，提交到方法
                // 可选的 async:false,阻塞的异步就是同步
                beforeSend: function () {                   // do something.
                    // 一般是禁用按钮等防止用户重复提交
                    //$("#btnClick").attr({disabled:"disabled"});
